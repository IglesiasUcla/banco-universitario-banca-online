...
const Row = ({ contact, index, showActions = true }) => {
  ...
  const handleTransfer = () => {
    navigate("/transferencias/create", { state: { accountNumber: contact.account_number } });
  };
  ...
  return (
    ...
    >
      <span id={`contact-table-row-${index}-alias`} className="w-[20%]">
      ...
      <span id={`contact-table-row-${index}-description`} className="w-[20%]">
        {width <= 1024 ? cutText(contact.description, 15) : contact.description}
      </span>
      {showActions && (
        <div id={`contact-table-row-${index}-buttons`} className="w-[20%] h-full">
          {width <= 544 ? (
            <div className="w-full h-full flex justify-center items-center">
              <button className="has-tooltip" onClick={handleIsOpen}>
                <span className="tooltip text-xs bg-white p-1 -mt-4 rounded">
                  Opciones
                </span>
                <FaEllipsisVertical className="text-white bg-[#49beb7] hover:bg-[#24837c] h-6 w-6 p-1 rounded" />
                {isOpen && (
                  <div className="absolute right-0 mt-2 py-2 w-48 flex flex-col bg-white rounded-md shadow-md z-50">
                    <button onClick={toEditContact} className="text-left hover:bg-gray-100 py-1 px-2">
                      Editar
                    </button>
                      <button onClick={handleTransfer} className="text-left hover:bg-gray-100 py-1 px-2">
                        Transferir
                      </button>
            ...
            <div className="w-full h-full flex justify-end items-center space-x-2">
              <button className="has-tooltip" onClick={toEditContact}>
                <span className="tooltip text-xs bg-white p-1 -mt-0.5 lg:-mt-4 rounded">Editar</span>
                <FaEdit className="text-white bg-[#49beb7] hover:bg-[#24837c] h-6 w-6 p-1 rounded" />
              </button>
              <button className="has-tooltip" onClick={handleTransfer}>
                <span className="tooltip text-xs bg-white p-1 -mt-0.5 lg:-mt-4 rounded">Transferir</span>
                <FaMoneyBillTransfer className="text-white bg-[#49beb7] hover:bg-[#24837c] h-6 w-6 p-1 rounded" />
              </button>
              <button className="has-tooltip" onClick={handleOpenModal}>
                <span className="tooltip text-xs bg-white p-1 -mt-0.5 lg:-mt-4 rounded">Eliminar</span>
                <FaRegTrashAlt className="text-white bg-[#49beb7] hover:bg-[#24837c] h-6 w-6 p-1 rounded" />
              </button>
            </div>
          )}
          <Modal title="¿Está seguro de eliminar este contacto?" show={showModal} onClose={handleCloseModal}>
            <div id="modal-content" className="space-y-4">
              ...
              <div id="modal-buttons" className="w-full flex justify-end space-x-4">
                <StyledButton
                  label="Cancelar"
