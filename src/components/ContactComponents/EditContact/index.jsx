...
const EditContactForm = () => {
  ...
  const getContact = async () => {
    try {
      ...
      showErrorToast("No fue posible obtener el contacto. Intente más tarde. " + error);
      handleBack();
    }
  }
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      const { data } = await contactAPI.update(id, newContactAdapter(values));

      if (!data?.data || data?.errors.length > 0) {
        const message = data?.message ?? "No es posible editar al contacto en este momento.";
      ...
      showErrorToast(`Error en edición de contacto: ${error}`);
      console.error("Error edición de contacto:", error);
    } finally {
      stopLoading();
    }
  }
